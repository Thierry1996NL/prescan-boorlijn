...
export default function Stap8_3D({ project }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Camera: volledige vrijheid alle richtingen
      ...
      // ── 3D BAG via PDOK WFS (CORS ingeschakeld, geen proxy nodig) ──
      ...
      (async()=>{
        try {
          if(boorCoords.length>=2){
            ...
            const wfsUrl=`https://service.pdok.nl/lv/bag/wfs/v2_0?SERVICE=WFS&VERSION=2.0.0&REQUEST=GetFeature&TYPENAMES=bag:pand&BBOX=${bbox},urn:ogc:def:crs:OGC:1.3:CRS84&OUTPUTFORMAT=application/json&COUNT=500`;
            const res=await fetch(wfsUrl);
            if(!res.ok)throw new Error(`PDOK WFS ${res.status}`);
            const data=await res.json();
            const pp2=ahnProfiel?.profielPunten??[];
            const baseH=napNaarCesium(pp2[Math.floor(pp2.length/2)]?.hoogte??0);
            let teller=0;
            (data.features??[]).forEach(feat=>{
              ...
              viewer._bag3dEntities.push(ent);teller++;
            });
            console.log(`[3D BAG PDOK] ${teller} gebouwen`);
            setBag3dStatus(teller>0?'klaar':'leeg');
            setBag3dTeller(teller);
        ...
        <div className="w-52 flex-shrink-0 bg-white border border-gray-200 rounded-xl p-4 flex flex-col gap-3">
          ...
          <div className="border-t border-gray-100 pt-2">
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={!!lagen.bag3d}
                onChange={e=>setLagen(p=>({...p,bag3d:e.target.checked}))}
              ...
              {bag3dStatus==="klaar"&&<span className="text-xs text-green-500">🏠 {bag3dTeller}</span>}
              {bag3dStatus==="leeg"&&<span className="text-xs text-amber-500">⚠ 0</span>}
            </label>
            {bag3dStatus==="leeg"&&<div className="text-xs text-amber-600 ml-6 leading-tight">Geen gebouwen gevonden in dit gebied.</div>}
            <label className="flex items-center gap-2 cursor-pointer">
