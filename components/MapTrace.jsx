...
// BGT WFS lagen met juiste namespace en property namen
const BGT_LAGEN = [
  ...
  for (const { typeName, prop } of BGT_LAGEN) {
    try {
      const params = new URLSearchParams({
        ...
        request: "GetFeature",
        typeName,
        bbox: `${miny},${minx},${maxy},${maxx},urn:ogc:def:crs:EPSG::4326`,
      ...
      if (json.features?.length > 0) {
        ...
        if (type && type !== "transitie") {
          return { laag: typeName, type };
        }
      }
    } catch (e) {
      console.error("BGT WFS fout:", typeName, e);
    }
  }
  return null;
}
...
function vertaalOppervlak(type) {
  if (!type) return { label: "Onbekend", kleur: "#9ca3af", icoon: "❓" };
  const lc = type.toLowerCase();

  if (lc.includes("gesloten")) return { label: "Asfalt / Beton", kleur: "#374151", icoon: "🛣" };
  if (lc.includes("open verharding")) return { label: "Klinkers / Tegels", kleur: "#6b7280", icoon: "🧱" };
  if (lc.includes("half verhard")) return { label: "Half verhard", kleur: "#92400e", icoon: "🪨" };
  if (lc.includes("onverhard")) return { label: "Onverhard", kleur: "#78350f", icoon: "🌱" };
  if (lc.includes("gras")) return { label: "Grasberm", kleur: "#16a34a", icoon: "🌿" };
  if (lc.includes("groenvoorziening")) return { label: "Groen / Plantsoen", kleur: "#15803d", icoon: "🌳" };
  if (lc.includes("struiken")) return { label: "Struiken", kleur: "#166534", icoon: "🌿" };
  if (lc.includes("bos")) return { label: "Bos", kleur: "#14532d", icoon: "🌲" };
  if (lc.includes("zand")) return { label: "Zand", kleur: "#d97706", icoon: "🏖" };
  if (lc.includes("riet") || lc.includes("moeras")) return { label: "Riet / Moeras", kleur: "#0369a1", icoon: "🌾" };
  if (lc.includes("meer") || lc.includes("sloot") || lc.includes("kanaal") || lc.includes("rivier") || lc.includes("water")) return { label: "Water", kleur: "#0284c7", icoon: "💧" };
  if (lc.includes("spoor")) return { label: "Spoor", kleur: "#dc2626", icoon: "🚆" };

  return { label: type, kleur: "#6b7280", icoon: "📍" };
}
...
export default function MapTrace({ project, onTraceOpgeslagen }) {
  ...
  function initKaart() {
    ...
    L.tileLayer("https://service.pdok.nl/brt/achtergrondkaart/wmts/v2_0/standaard/EPSG:3857/{z}/{x}/{y}.png",
      { attribution: "© PDOK BRT", maxZoom: 19 }).addTo(kaart);
    ...
    kaart.on("click", async (e) => {
      ...
      const vertaald = result ? vertaalOppervlak(result.type) : { label: "Geen BGT data", kleur: "#9ca3af", icoon: "❓" };

      L.circleMarker([lat, lng], {
        radius: 7, fillColor: vertaald.kleur, color: "#fff", weight: 2, fillOpacity: 1,
      })
        ...
        <div className="w-64 flex-shrink-0 flex flex-col gap-3">
          ...
          {uniekeOppervlakken.length > 0 && (
            <div className="bg-blue-50 border border-blue-100 rounded-xl p-3">
              <p className="text-xs text-blue-700">💡 Sla het tracé op en vraag de AI Assistent om hersteladvies per oppervlaktype.</p>
            </div>
          )}
        </div>
      )}
