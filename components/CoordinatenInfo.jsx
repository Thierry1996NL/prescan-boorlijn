...
const ONDERLEGGERS = [
  // PDOK
  { groep:"PDOK", naam:"BRT Standaard",    type:"WMTS", crs:"EPSG:28992", url:"service.pdok.nl/brt/…/standaard", opmerking:"Native RD New WMTS. Geonovum richtlijn 72 DPI. Geen conversie nodig.", stappen:[3,4,5,6,7,8] },
  { groep:"PDOK", naam:"BRT Grijs",        type:"WMTS", crs:"EPSG:28992", url:"service.pdok.nl/brt/…/grijs",     opmerking:"Native RD New WMTS. Identiek tile-schema als BRT Standaard.",          stappen:[3,4,5,6,7,8] },
...
function LagenTabel({ items, stap }) {
  ...
  // Group by groep
  const groepen = [...new Set(relevant.map(i => i.groep ?? "PDOK"))];
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
      {groepen.map(groep => {
        const groepItems = relevant.filter(i => (i.groep ?? "PDOK") === groep);
        return (
          <div key={groep}>
            <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:4 }}>
              <span style={{ fontSize:10, fontWeight:700, color: groep==="Esri NL"?"#5B7FA6":"#8FA6B2", textTransform:"uppercase", letterSpacing:".05em" }}>{groep}</span>
              {groep==="Esri NL"&&<span style={{ fontSize:10, background:"#EEF5FF", color:"#5B7FA6", border:"1px solid #C5DEFF", borderRadius:10, padding:"0 6px", fontWeight:600 }}>WMS · EPSG:28992</span>}
              <div style={{ flex:1, height:1, background:"#F0F4F6" }}/>
            </div>
            {groepItems.map((item, i) => {
