...
const STAPPEN = [
  { nr: 1,  label: "Projectinformatie",  icon: "📋", sub: "Basisgegevens",           leeg: false },
  { nr: 2,  label: "Ontwerp inladen",    icon: "📂", sub: "LS · MS · Gas · Water",   leeg: false },
  { nr: 3,  label: "Boorlijn tekenen",   icon: "✏️", sub: "Tracé op de kaart",       leeg: false },
  { nr: 4,  label: "Oppervlakteanalyse", icon: "🗺️", sub: "BGT oppervlakken",        leeg: false },
  { nr: 5,  label: "Diepteligging",      icon: "📐", sub: "Dwarsprofiel & bodem",    leeg: false },
  { nr: 6,  label: "Machine locatie",    icon: "🏗️", sub: "Boormachine & bentoniet", leeg: false },
  { nr: 7,  label: "Eindontwerp",        icon: "✅", sub: "Overzicht read-only",     leeg: false },
  { nr: 8,  label: "AI optimalisatie",   icon: "🤖", sub: "Kruisingen & advies",     leeg: true  },
  { nr: 9,  label: "Pre-scan",           icon: "🔍", sub: "Tekeningen",              leeg: true  },
  { nr: 10, label: "Contactpersonen",    icon: "👥", sub: "KLIC contacten",          leeg: true  },
  { nr: 11, label: "Exporteren",         icon: "📤", sub: "Rapport & coördinaten",   leeg: true  },
];
...
export default function Sidebar({
  ...
  async function laadProjecten() {
    try {
      ...
      console.error("Sidebar – projecten laden:", err);
    }
  }
  ...
  // Gebruiker initialen voor de gele cirkel
  const initialen = (() => {
    if (gebruiker?.naam) {
  ...
  const weergaveNaam = gebruiker?.naam || gebruiker?.email || "Gebruiker";
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-4 border-b border-gray-100 flex-shrink-0">
        ...
        <div className={`flex items-center gap-1 ${ingeklapt ? "w-full justify-center" : "ml-auto"}`}>
          ...
          <button
            ...
            className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-gray-100 text-gray-400 hover:text-gray-600 transition-colors text-sm"
      ...
      >
        {/* Gele cirkel */}
        <div className="w-8 h-8 rounded-full bg-yellow-400 flex items-center justify-center text-xs font-bold text-yellow-900 flex-shrink-0 select-none shadow-sm">
          {initialen}
        </div>
        {!ingeklapt && (
          <div className="min-w-0">
            <div className="text-xs font-medium text-gray-800 truncate">{weergaveNaam}</div>
            {gebruiker?.naam && (
      ...
      <div className="flex-1 py-3 overflow-y-auto">
        {inProject ? (
          /* ── Project modus: 11 stappen ─────────────────── */
          <>
            {/* Terug naar projecten */}
            <button
              onClick={() => router.push("/projecten")}
              className={`flex items-center gap-2 w-full px-4 py-2 text-xs text-gray-400 hover:text-gray-600 hover:bg-gray-50 transition-colors mb-2 ${
                ingeklapt ? "justify-center" : ""
              }`}
            >
            ...
            <div className="px-2 space-y-0.5">
              {STAPPEN.map((stap) => {
                ...
                const voltooid = actieveStap > stap.nr && !stap.leeg;
                ...
                return (
                  ...
                  >
                      ...
                      <div className="min-w-0 flex-1">
                        <div
                          className={`text-xs font-medium truncate ${
                            actief ? "text-orange-700" : "text-gray-700"
                          }`}
                        >
                          {stap.label}
                        </div>
                    ...
                    {!ingeklapt && stap.leeg && (
                      <span className="text-xs text-gray-300 flex-shrink-0 italic">binnenkort</span>
                    )}
                  </button>
                );
              })}
          ...
          /* ── Standaard modus: projectenlijst ────────────── */
          <>
            {!ingeklapt && (
              ...
              {projecten.map((p) => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-2 w-full px-3 py-2 rounded-lg text-left transition-colors text-sm ${
                      actief
      ...
      <div className="px-3 py-3 border-t border-gray-100 flex-shrink-0">
        <button
          ...
          className={`flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-gray-400 hover:text-gray-700 hover:bg-gray-100 transition-colors w-full ${
            ingeklapt ? "justify-center" : "text-left"
          }`}
        >
          <span className="flex-shrink-0">↩</span>
