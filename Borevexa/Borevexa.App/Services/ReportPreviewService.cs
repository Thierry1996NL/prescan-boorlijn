...
public sealed class ReportPreviewService
{
        ...
        string? variantKey = null)
    {
        ...
        finally
        {
            ...
            {
                try { File.Delete(tempPath); } catch (System.Exception swallowedException)
        {
    ...
    {
        return Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Borevexa", "ReportLiveMaps");
    }
    ...
    private static void CleanupOldLiveMapCaptures(string directory, string projectId, int stepNumber)
    {
        try
        {
            ...
            {
                try { file.Delete(); } catch (System.Exception swallowedException)
        {
            AppLog.Swallowed(swallowedException);
        }
            }
